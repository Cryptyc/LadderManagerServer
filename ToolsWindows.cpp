...
void StartBotProcess(const BotConfig &Agent, const std::string &CommandLine, unsigned long *ProcessId)
{
	...
	if (!result)
	{
		...
		std::cout << "Starting bot: " << Agent.BotName << " with command:" << std::endl << CommandLine << " failed. Error " << dw << " : " << message << std::endl;
	...
	{
		std::cout << "Starting bot: " << Agent.BotName << " with command:" << CommandLine << std::endl;
